 ...
 * <p>
 * A checked exception represents invalid conditions in areas outside the
 * immediate control of the program (invalid user input, database problems,
 * network outages, absent files) are subclasses of {@link Exception}.
 * <p>
 * A method is obliged to establish a policy for all checked exceptions thrown
 * by its implementation (either pass the checked exception further up the
 * stack, or handle it somehow).
 * <p>
 * It is somewhat confusing, but note as well that {@link RuntimeException}
 * (unchecked) is itself a subclass of {@link Exception} (checked).
 * <hr>
...
public abstract class AbstractCheckedException extends Exception
{
	...
	/**
	 * Creates a new checked empty exception.
	 */
	...
	/**
	 * Creates a new checked exception based on a message and a parent
	 * exception.
	 * <hr>
	 * @param message Message of the exception.
	 * @param exception Parent exception.
	...
	/**
	 * Creates a new checked exception based on an exception enumerated value
